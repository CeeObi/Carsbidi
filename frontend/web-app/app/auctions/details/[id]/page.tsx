import { getBidsForAuctions, getDetailedAuction } from '@/app/actions/auctionActions'
...
async function Details({params}: {params: {id : string}}) {
  ...
  const bids = getBidsForAuctions(params.id)
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-6 mt-3">
        ...
        <div className="border-2 rounded-lg p-2 bg-gray-100">
          <Headings title='Bids'/>
          {(await bids).map((bid) => <BidItem key={bid.id} bid={bid} />)}
        </div>
      </div>
